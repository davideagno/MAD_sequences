...
pred_regr_one <- function(y, X, alpha, lambda, N_star, P_0, sigma, delta_x, support_y) {
  P_0 <- c(P_0)
  d <- NCOL(X) + 1
...
pred_regr <- function(y, X, alpha, lambda, N_star, P_0, hyper, support_y, nPerm, cores = 1, seed = 1) {
  require(parallel)
  N <- length(y)
  ...
  mclapply_function <- function(p) {
    pred_regr_one(y[idx_perm[,p]], X[idx_perm[,p],], alpha, lambda, N_star, P_0, sigma, delta_x, support_y)
  }
...
preq_regr_one <- function(y, X, alpha, lambda, N_star, P_0, sigma, delta_x, support_y) {
  P_0 <- c(P_0)
  d <- NCOL(X) + 1
  lsy <- length(support_y)
...
preq_regr <- function(y, X, alpha, lambda, N_star, P_0, hyper, support_y, nPerm, cores = 1, seed = 1) {
  require(parallel)
  N <- length(y)
  idx_perm <- matrix(nrow = N, ncol = nPerm)
  for (p in 1:nPerm) {
  ...
  delta_x <- hyper[2]
  mclapply_function <- function(p) {
    preq_regr_one(y[idx_perm[,p]], X[idx_perm[,p],], alpha, lambda, N_star, P_0, sigma, delta_x, support_y)
  }
  out <- mclapply(1:nPerm, mclapply_function, mc.cores = cores)
  mean(unlist(out))
}

preq_cond_regr_one <- function(y, X, alpha, lambda, N_star, P_0, sigma, delta_x, support_y) {
  P_0 <- c(P_0)
  d <- NCOL(X) + 1
  lsy <- length(support_y)
...
preq_cond_regr <- function(y, X, alpha, lambda, N_star, P_0, hyper, support_y, nPerm, cores = 1, seed = 1) {
  require(parallel)
  N <- length(y)
  idx_perm <- matrix(nrow = N, ncol = nPerm)
  for (p in 1:nPerm) {
  ...
  delta_x <- hyper[2]
  mclapply_function <- function(p) {
    preq_cond_regr_one(y[idx_perm[,p]], X[idx_perm[,p],], alpha, lambda, N_star, P_0, sigma, delta_x, support_y)
  }
  out <- mclapply(1:nPerm, mclapply_function, mc.cores = cores)
  mean(unlist(out))
}
...
pred_res_regr_one <- function(p_n_vec, dim_joint_supp, sample_size, alpha, lambda, N_star, sigma, delta_x, support_y, ball, interval, N, seed = 1) {
  lsy <- length(support_y)
  set.seed(seed)
  for (n in 1:N) {
...
pred_res_regr <- function(p_n, sample_size, alpha, lambda, N_star, sigma, delta_x, support_y, N, B, cores = 1, seed = 1, dp = FALSE) {
  require(parallel)
  p_n <- c(p_n)
  djs <- length(p_n)
  ...
  mclapply_function <- function(b) {
    seed_b <- seed + b
    pred_res_regr_one(p_n, djs, sample_size, alpha, lambda, N_star, sigma, delta_x, support_y, ball, interval, N, seed_b)
  }
