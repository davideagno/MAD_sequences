...
pred_class_one <- function(y, X, alpha, lambda, P_0, delta_y, delta_x) {
  P_0 <- c(P_0)
  for (n in 1:NROW(y)) {
    y_obs <- y[n]
    x_obs <- c(X[n,])
...
pred_class <- function(y, X, alpha, lambda, P_0, delta, nPerm, cores = 1, seed = 1) {
  require(parallel)
  N <- length(y)
  ...
  mclapply_function <- function(p) {
    pred_class_one(y[idx_perm[,p]], X[idx_perm[,p],], alpha, lambda, P_0, delta_y, delta_x)
  }
...
preq_cond_class_one <- function(y, X, alpha, lambda, P_0, delta_y, delta_x) {
  P_0 <- c(P_0)
  preq <- 0
  for (n in 1:NROW(y)) {
    y_obs <- y[n]
...
preq_cond_class <- function(y, X, alpha, lambda, P_0, delta, nPerm, cores = 1, seed = 1) {
  require(parallel)
  N <- length(y)
  idx_perm <- matrix(nrow = N, ncol = nPerm)
  for (p in 1:nPerm) {
  ...
  delta_x <- delta[2]
  mclapply_function <- function(p) {
    preq_cond_class_one(y[idx_perm[,p]], X[idx_perm[,p],], alpha, lambda, P_0, delta_y, delta_x)
  }
  out <- mclapply(1:nPerm, mclapply_function, mc.cores = cores)
  mean(unlist(out))
}

preq_class_one <- function(y, X, alpha, lambda, P_0, delta_y, delta_x) {
  P_0 <- c(P_0)
  preq <- 0
  for (n in 1:NROW(y)) {
    y_obs <- y[n]
...
preq_class <- function(y, X, alpha, lambda, P_0, delta, nPerm, cores = 1, seed = 1) {
  require(parallel)
  N <- length(y)
  idx_perm <- matrix(nrow = N, ncol = nPerm)
  for (p in 1:nPerm) {
  ...
  delta_x <- delta[2]
  mclapply_function <- function(p) {
    preq_class_one(y[idx_perm[,p]], X[idx_perm[,p],], alpha, lambda, P_0, delta_y, delta_x)
  }
  out <- mclapply(1:nPerm, mclapply_function, mc.cores = cores)
  mean(unlist(out))
}
...
pred_res_class_one <- function(p_n_vec, dim_joint_supp, sample_size, alpha, lambda, delta_y, delta_x, N, seed = 1) {
  set.seed(seed)
  for(n in 1:N) {
...
pred_res_class <- function(p_n, sample_size, alpha, lambda, delta, N, B, cores = 1, seed = 1) {
  require(parallel)
  p_n <- c(p_n)
  djs <- length(p_n)
  delta_y <- delta[1]
  ...
  mclapply_function <- function(b) {
    seed_b <- seed + b
    pred_res_class_one(p_n, djs, sample_size, alpha, lambda, delta_y, delta_x, N, seed_b)
  }
